...
const convertedFromDB = async (allData, desiredTimeZone, desiredDate) => {
  // const items = await allData.map(item => {
  //   if (item.fields.Date && item.fields.Time) {
  //     const swedenDateTime = `${item.fields.Date} ${item.fields.Time}`;
  //     const convertedDateTime = moment2.tz(swedenDateTime, 'YYYY-MM-DD HH:mm', 'Europe/Stockholm').tz(desiredTimeZone);
  ...
  for (const item of allData) {
    if (item.fields.Date && item.fields.Time) {
      ...
      if (convertedDateTime.format("YYYY-MM-DD") !== desiredDate) {
        continue;
      }
