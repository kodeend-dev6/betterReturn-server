...
const groupDataByDate = (data, dateFormat, game) => {
  ...
  Object.keys(groupedByDate).forEach((date) => {
    ...
    groupedByDate[date].accuracy = (
      (groupedByDate[date].accuracy / totalMatches) *
      100
    ).toFixed(2);
  });
...
const groupDataByWeek = (data, dateFormat, game) => {
  ...
  Object.keys(groupedByWeek).forEach((week) => {
    ...
    groupedByWeek[week].accuracy = (
      (groupedByWeek[week].accuracy / totalMatches) *
      100
    ).toFixed(2);
  });
...
const groupDataByMonth = (data, dateFormat, game) => {
  ...
  Object.keys(groupedByMonth).forEach((month) => {
    ...
    groupedByMonth[month].accuracy = (
      (groupedByMonth[month].accuracy / totalMatches) *
      100
    ).toFixed(2);
  });
...
const getDashboardData = catchAsync(async (req, res) => {
  ...
  if (game === "csgo") {
    try {
      const escapedLeagueName =
        league?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") || "";
      const escapedTeamName =
        team?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") || "";
      const escapedCountryName =
        country?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") || "";
      ...
      const totalWinMatch = allData.filter(
        (m) => m.fields.Prediction === m.fields.Results
      );
    ...
    try {
      ...
      const totalWinMatch = allData?.filter(
        (m) => m.fields.Result === m.fields.Prediction
      );
      const winMatch = totalWinMatch?.length;
      const lostMatch = allData?.length - winMatch;
      const total = winMatch + lostMatch;
